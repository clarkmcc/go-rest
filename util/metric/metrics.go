...
func (n noopLatency) Observe(verb string, url url.URL, duration time.Duration) {
	n.verb = verb
	n.url = url
	n.duration = duration
}

func (n noopLatency) String() string {
	return fmt.Sprintf("'%v' request to %v took %s", n.verb, n.url, n.duration)
}
