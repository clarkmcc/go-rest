...
// A RESTClient created by this method is generic - it expects to operate on an API that follows
// the Kubernetes conventions, but may not be the Kubernetes API.
func GenericClientFor(config *Config) (*RESTClient, error) {
	if config.ContentConfig.Marshaller == nil {
		return nil, fmt.Errorf("contentConfig.Marshaller is required")
	}
	...
	if config.ContentConfig.GroupVersion == nil {
		config.ContentConfig.GroupVersion = &clientcmdapi.SchemeGroupVersion
	}

	baseURL, versionedAPIPath, err := defaultUnversionedServerUrlFor(config)
	if err != nil {
		return nil, err
	}
