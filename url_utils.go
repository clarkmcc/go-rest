...
// DefaultUnversionedServerURL converts a host, host:port, or URL string to the default base server API path
// to use with a Client at a given API version following the standard conventions for a
// Kubernetes API.
func DefaultUnversionedServerURL(host, apiPath string, defaultTLS bool) (*url.URL, string, error) {
	if host == "" {
		return nil, "", fmt.Errorf("host must be a URL or a host:port pair")
	}
	base := host
	...
	//
	// versionedAPIPath, a path relative to baseURL.Path, points to a versioned API base
	versionedAPIPath := DefaultUnversionedAPIPath(apiPath)
...
// DefaultUnversionedAPIPathFor constructs the default path, assuming the given
// API path, following the standard conventions of the Kubernetes API.
func DefaultUnversionedAPIPath(apiPath string) string {
	versionedAPIPath := path.Join("/", apiPath)
	return versionedAPIPath
}
...
// defaultUnversionedServerUrlFor is shared between IsConfigTransportTLS and RESTClientFor. It
// requires Host to be set prior to being called.
func defaultUnversionedServerUrlFor(config *Config) (*url.URL, string, error) {
	// TODO: move the default to secure when the apiserver supports TLS by default
	// config.Insecure is taken to mean "I want HTTPS but don't bother checking the certs against a CA."
	hasCA := len(config.CAFile) != 0 || len(config.CAData) != 0
	hasCert := len(config.CertFile) != 0 || len(config.CertData) != 0
	...
	return DefaultUnversionedServerURL(host, config.APIPath, defaultTLS)
}
